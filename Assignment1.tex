...
\begin{align}\label{eqn1}
    \cos(\theta) = \dfrac{\vec{F} \cdot \vec{d}}{\norm{\vec{F}} \norm{\vec{d}}}
\end{align}
Here, the scalar product of the vectors $\vec{F}$ and $\vec{d}$ can be given as,
\begin{align}\label{eqn2}
    \vec{F} \cdot \vec{d} = \vec{F}^T\vec{d}
\end{align}

\section{Solution}

Using Equation \ref{eqn2} and substituting the values,
\[ \vec{F} \cdot \vec{d} = \myvec{3 \\ 4 \\ -5}^T \myvec{5 \\ 4\\ 3} \]
\[ \implies\vec{F} \cdot \vec{d} = \myvec{3 & 4 & -5} \myvec{5 \\ 4 \\ 3} = 16 \]
...
\[ \implies\theta = \arccos ({\dfrac{8}{25}})\\ \]
\[ \implies\theta \approx \ 71.3\degree \]
Thus, the angle between $\vec{F}$ and $\vec{d}$ is approximately 71.3\degree
\end{document}